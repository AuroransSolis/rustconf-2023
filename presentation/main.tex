...
\begin{document}
	...
	\begin{frame}[fragile]{im begging you dont do this}
		...
		\begin{onlyenv}<2>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {1}]{rust}
				trait_xml_inner! {
					@parse {
						input: [
							<trait>
		...
		\begin{onlyenv}<3>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {2}]{rust}
				trait_xml_inner! {
					@parsetrait {
						input: [
								<name>Foo</name>
		...
		\begin{onlyenv}<4>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {1, 10-16}]{rust}
		...
		\begin{onlyenv}<5>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {2, 11-15}]{rust}
				trait_xml_parse_name_ident! {
					@parseend {
						input: [
								</name>
		...
		\begin{onlyenv}<6>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {1, 2, 9}]{rust}
				trait_xml_inner! {
					@namecallback {
						input: [
								<vis>pub</vis>
		...
		\begin{onlyenv}<7>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {2, 8}]{rust}
				trait_xml_inner! {
					@parsetrait {
						input: [
								<vis>pub</vis>
		...
		\begin{onlyenv}<8>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {1, 8-14}]{rust}
				trait_xml_parse_vis! {
					@parse {
						input: [
								pub</vis>
		...
		\begin{onlyenv}<10>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {2, 7}]{rust}
				trait_xml_inner! {
					@parsetrait {
						input: [
								<unsafe/>
		...
		\begin{onlyenv}<11>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {3}]{rust}
				trait_xml_inner! {
					@parsetrait {
						input: [</trait>],
		...
		\begin{onlyenv}<13>
			...
			\begin{minted}[autogobble, linenos = false, highlightlines = {6}]{rust}
				trait_xml_inner! {
					@expand {
		...
		\begin{onlyenv}<14>
			\begin{minted}[autogobble, linenos = false, highlightlines = {4}]{rust}
				trait_xml_inner! {
					@expand {
						output: [[unsafe]],
						vis: [pub],
	...
	\begin{frame}[fragile]{Incremental TT munchers my beloved}
		...
		\begin{minted}[autogobble, highlightlines = {2, 5}, linenos = false]{rust}
			macro_rules! some_macro {
				() => {
					/* expansion */
				};
	...
	\begin{frame}[fragile]{Callbacks my beloved}
		\pdfpcnote{
			- In the XML parsing macros this was used to allow for macro reuse.\\
			- For instance, every `<name>` section was parsed by one macro\\
			- This allows you to have single points of failure and reduces maintenance burden
		}
